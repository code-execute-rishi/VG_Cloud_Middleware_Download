...
func main() {
	log.Println("[Camera] Starting Vyom Camera Service (Robust Go-Framing)...")

	go runGStreamerPipeline()

	http.HandleFunc("/", handleStream)

	log.Printf("[Camera] Serving Real Camera Stream on %s", Port)
...
func runGStreamerPipeline() {
	...
	for {
		...
		if duration < 15*time.Second || bytesReadTotal == 0 {
			consecutiveFailures++
			log.Printf("[Warn] Pipeline failed quickly (%v). Failures: %d", duration, consecutiveFailures)
